...
from scraper.collector import collect_categories, collect_recipe_ids
...
def build_parser() -> argparse.ArgumentParser:
    ...
    ids_parser.add_argument("--delay", type=float, default=0.35)
    ids_parser.add_argument("--workers", type=int, default=4)

    scrape_parser = subparsers.add_parser("scrape", help="Спарсить рецепты по ID")
    scrape_parser.add_argument("--delay", type=float, default=0.35)
    scrape_parser.add_argument("--workers", type=int, default=6)
    ...
    all_parser.add_argument("--delay", type=float, default=0.35)
...
def main() -> None:
    ...
    if args.command == "ids":
        ...
        stats = collect_recipe_ids(client, categories_path, ids_path, previews_path, workers=args.workers)
        print(stats)
        return
    ...
    if args.command == "scrape":
        if not ids_path.exists():
            if not categories_path.exists():
                collect_categories(client, categories_path)
            collect_recipe_ids(client, categories_path, ids_path, previews_path, workers=min(args.workers, 4))
    ...
    if args.command == "all":
        ...
        stats_ids = collect_recipe_ids(client, categories_path, ids_path, previews_path, workers=min(args.workers, 4))
        print(stats_ids)
